 ...
 class UserController extends Controller
{
    ...
    public function indexUsers()
    {
        ...
        //$educationCompletion = DataArrayHelper::defaultProfileEducationArray();
         return view('admin.user.index')->with('jobExperiences', $jobExperiences)
    ...
    public function storeUser(UserFormRequest $request)
    {
        ...
        $user->degree_level_id = $request->input('degree');
    ...
    public function fetchUsersData(Request $request)
    {
        ...
        return Datatables::of($users)
                        ->filter(function ($query) use ($request) {
                            ...
                            if ($request->has('phone') && !empty($request->phone)) {
                             ...
                             if ($request->has('degree_level_id') && !empty($request->degree_level_id)) {
                                $query->where('users.degree_level_id', $request->get('degree_level_id'));
                            }

                             if ($request->has('job_experience_id') && !empty($request->job_experience_id)) {
                                $query->where('users.job_experience_id', $request->get('job_experience_id'));
                            }
                            if ($request->has('functional_area_id') && !empty($request->functional_area_id)) {
                                $query->where('users.functional_area_id', $request->get('functional_area_id'));
                            }
                            if ($request->has('passed_out_year') && !empty($request->passed_out_year)) {
                                $query->where('users.id', $request->get('passed_out_year'));
                            }

                            if ($request->has('current_salary') && !empty($request->current_salary)) {
                                $query->where('users.current_salary', 'like', "{$request->get('current_salary')}");
                            }
                            if ($request->has('expected_salary') && !empty($request->expected_salary)) {
                                $query->where('users.expected_salary', 'like', "{$request->get('expected_salary')}");
                            }
                        })
                        ...
                        ->addColumn('phone', function ($users) {
                          ...
                          ->addColumn('degree_level', function ($users) {
                            if(isset($users->getProfileEducation()[0]))
                              return $users->getProfileEducation()[0]->degree_title;
                              else
                                return '';
                             //return $users->getDegreeLevel('degree_level');
                         })
                        ->addColumn('job_experience', function ($users) {
                            return $users->getJobExperience('job_experience');
                        })
                        ->addColumn('functional_area', function ($users) {
                            return $users->getFunctionalArea('functional_area');
                        })
                         ->addColumn('passed_out_year', function ($users) {
                            if(isset($users->getProfileEducation()[0]))
                              return $users->getProfileEducation()[0]->date_completion;
                              else
                                return '';
                        })
                        // ->addColumn('passed_out_year', function ($users) {
                        //     return $users->getDateCompletion('date_completion');
                        // })
                        ->addColumn('current_salary', function ($users) {
                            return $users->current_salary;
                        })->addColumn('expected_salary', function ($users) {
                            return $users->expected_salary;
                        })
